...
<section class="flex flex-col md:flex-row items-center justify-between px-6 md:px-10 py-16 md:py-32 bg-gradient-to-r from-[#FF3D3B] via-[#FF5A4B] to-[#FF7A66] text-white gap-10">
    ...
    <div x-data="carousel()" x-init="start()" class="w-full md:w-1/2 relative overflow-hidden">
        ...
        <button @click="prev()" class="cursor-pointer absolute left-2 top-1/2 -translate-y-1/2 bg-white text-[#FF5A4B] rounded-full p-2 shadow hover:bg-gray-200 transition">
            <i class="bi bi-chevron-left text-2xl"></i>
        </button>
        <button @click="next()" class="cursor-pointer absolute right-2 top-1/2 -translate-y-1/2 bg-white text-[#FF5A4B] rounded-full p-2 shadow hover:bg-gray-200 transition">
    ...
    <section class="px-6 md:px-10 py-16">
        ...
        <div class="flex gap-6 overflow-x-auto pb-4">
            @foreach([
                ['icon'=>'bi-check-circle-fill','title'=>'Cadastro de Produtos','text'=>'Adicione imagens, nomes, preços, categorias e estoque de forma rápida e fácil.'],
                ['icon'=>'bi-people-fill','title'=>'Gestão de Usuários','text'=>'Controle acessos e permissões dos seus usuários de forma simples.'],
                ['icon'=>'bi-cart-fill','title'=>'Carrinho Inteligente','text'=>'Cálculo automático do total e gerenciamento do pedido em tempo real.'],
                ['icon'=>'bi-heart-fill','title'=>'Wishlist','text'=>'Salve seus produtos favoritos e acesse rapidamente quando quiser.'],
                ['icon'=>'bi-shield-lock-fill','title'=>'Segurança Avançada','text'=>'Autenticação confiável e proteção de dados para você e seus clientes.'],
                ['icon'=>'bi-headset','title'=>'Suporte Dedicado','text'=>'Equipe pronta para ajudar em qualquer problema de forma rápida e eficiente.']
            ] as $feature)
            <div class="flex-shrink-0 w-80 p-6 bg-white rounded-xl shadow hover:shadow-lg transition flex flex-col items-center text-center">
